...
import type { CreateTestFileInput } from "@/lib/api";
...
interface CreateTestFileModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateTestFileInput) => Promise<void>;
  folderName: string;
}

const CreateTestFileModal = ({
  isOpen,
  onClose,
  onSubmit,
  folderName,
}: CreateTestFileModalProps) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onFormSubmit = async (data: FormData) => {
    try {
      ...
      setError("root", {
        message: err instanceof Error ? err.message : "Failed to create test file",
      });
    }
  };
